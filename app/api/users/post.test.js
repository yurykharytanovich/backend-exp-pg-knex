...
describe('Users POST API', () => {
    ...
    describe('POST /users', () => {
        it('should add all users to database with consistent data', async () => {
            let responses;
            try {
                responses = await Promise.all(usersForAdding
    ...
    describe('POST /users/:userId/items', () => {
        it('should add problem surveys to user', async() => {
            ...
            let responses;
            try {
                responses = await Promise.all(usersIds
                    .map(userId => {
                        const itemsIds = userItemsIds.filter(uiid => userItems[uiid][uiscols.USER_ID] === userId)
